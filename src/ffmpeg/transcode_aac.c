 ...
 * Convert an input audio file to AAC using FFmpeg.
 ...
 * @param      audio_stream_offset  The index of the audio stream starting a 0
 * @param[out] audio_stream_idx     The index of the audio stream to transcode
 * @param[out] input_format_context Format context of opened file
 * @param[out] input_codec_context  Codec context of opened file
 * @return Error code (0 if successful)
 */
static int open_input_file(const char *filename,
                           const size_t audio_stream_offset,
                           ...
                           AVCodecContext **input_codec_context)
{
    ...
    if (*audio_stream_idx >= (*input_format_context)->nb_streams) {
        fprintf(stderr, "Expected an audio stream, but found none\n");
        avformat_close_input(input_format_context);
        return AVERROR_EXIT;
    }
 ...
 * @param[out] start_pts            The starting position
 * @param[out] end_pts              The ending position
 * @return Error code (0 if successful)
 */
 ...
 * @param      input_codec_context  Codec context of the input file
 * @param      audio_stream_idx     Index of the audio stream to focus on
 * @param      start_pts            The starting frame position
 * @param      end_pts              The ending frame position
...
static int decode_audio_frame(AVFrame *frame,
                              ...
                              AVCodecContext *input_codec_context,
                              const size_t audio_stream_idx,
                              const int64_t start_pts, const int64_t end_pts,
                              int *data_present, int *finished)
{
    ...
    if (error != AVERROR_EOF) {
        /* Skip all streams that aren't the audio stream we want */
 ...
 * @param      input_format_context Format context of the input file
 * @param      input_codec_context  Codec context of the input file
 * @param      audio_stream_idx     Index of the audio stream
 * @param      start_pts            The starting frame position
 * @param      end_pts              The ending frame position
 * @param      output_codec_context Codec context of the output file
                                         ...
                                         int *finished)
{
    ...
    if (decode_audio_frame(input_frame, input_format_context,
                           input_codec_context, audio_stream_idx,
                           start_pts, end_pts,
 ...
 * @param      pts                   Timestamp for audio frames
 * @param      output_format_context Format context of the output file
 * @param      output_codec_context  Codec context of the output file
 ...
 * @return Error code (0 if successful)
 */
static int encode_audio_frame(AVFrame *frame, int64_t *pts,
                              AVFormatContext *output_format_context,
                              AVCodecContext *output_codec_context,
                              int *data_present)
{
 ...
 * @return Error code (0 if successful)
 */
static int load_encode_and_write(AVAudioFifo *fifo, int64_t *pts,
                                 AVFormatContext *output_format_context,
                                 AVCodecContext *output_codec_context)
{
    ...
    if (encode_audio_frame(output_frame, pts, output_format_context,
...
int transcode_aac(const char *input_file, const char *output_file,
                  ...
                  const double start, const double end)
{
    size_t audio_stream_idx = 0;
    int64_t start_pts = 0, end_pts = 0, pts = 0;
    ...
    if (open_input_file(input_file, audio_track, &audio_stream_idx,
    ...
    while (1) {
        ...
        while (av_audio_fifo_size(fifo) >= output_frame_size ||
            ...
            if (load_encode_and_write(fifo, &pts, output_format_context,
                                      output_codec_context))
                goto cleanup;
        ...
        if (finished) {
            ...
            do {
                data_written = 0;
                if (encode_audio_frame(NULL, &pts, output_format_context,
                                       output_codec_context, &data_written))
                    goto cleanup;
            } while (data_written);
            break;
